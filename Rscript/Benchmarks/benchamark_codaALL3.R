...
for(f in 1:k_fold){
  ...
  sets = unique(round(seq(10,max_parts,length.out = num_sets)))
  ...
  perf = data.frame(Scenario = if_else(permute_labels,"Permuted","Empirical"),
                    ...
                    number_parts = nparts,number_ratios = length(cn) ,comp_time = compTime[3],
                    base_dims = ncol(train.data))
  benchmark = rbind(benchmark,perf)
  ...
  #   ## with feature selection
  #   ref = ff$procrust.ref
  #   trainx = data.frame(alr(trainx,ivar = ref))
  #   testx = data.frame(alr(testx,ivar = ref))
  # })
  #
  #   #ensemble = c("ranger")
  #   m1.plr = trainML_Models(trainLRs = trainx,
  #                           testLRs = testx,
  ...
  #
  #     gm = cv.clrlasso$lambda
  #     aa = cv.clrlasso$glmnet.fit
  #     aa = as.matrix(coef(cv.clrlasso, s = cv.clrlasso$lambda))
  #
  #
  #     features = as.matrix(coef(cv.clrlasso, s = "lambda.min"))
  #     features = features[-1,]
  ...
  # #Write Results
  # perf = data.frame(Scenario = if_else(permute_labels,"Permuted","Empirical"),
  #                   Dataset = f_name,Seed = sd,Fold = f,Approach = "ALR-LASSO",AUC = as.numeric(pROC::auc(mroc.glm.alr)),train_auc = NA,
  #                   number_parts = length(features)+1,number_ratios = length(features) ,comp_time = compTime[3]+compTime2[2],
  #                   base_dims = ncol(train.data)
  # )
  #
  # benchmark = rbind(benchmark,perf)
...
write_csv(x = benchmark,file = paste0("Results/Exp_Benchamark4/",f_name,".csv"))
