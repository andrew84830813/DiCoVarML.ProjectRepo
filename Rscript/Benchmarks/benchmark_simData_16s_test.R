...
for(f in 1:k_fold){
  ...
  dcv1 = dcv1 %>%
    filter(rowmean>0)
  dcv_strength = DiCoVarML::computeDCVStrength(list(dcv  =dcv1))
  dcv_strength$ss = scale(dcv_strength$Str)

  dcv_strength1 = dcv_strength %>%
    filter(ss>1.64)
  min = nrow(dcv_strength1)
