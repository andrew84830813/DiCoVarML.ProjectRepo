...
sd = 2
write_csv(performance,file = "Output/simplexLinearSim.csv")
...
for(parts in c(10,50,100,150)){


  comp =  compositions::rDirichlet.acomp(2,alpha = 20*c(rep(1,parts)));comp = data.frame(comp)[1,]


  ## Data Set 1
  x1 = (comp[1,])
  a = round(seq(0,100,by = .1),1)  #<===== Input =======
  madison = foreach(A = a,.combine = rbind)%dopar%{
    ph = data.frame((compositions::clo(x1^A)))
    data.frame(Dataset = 1,Status = A,ph)
  }
  alpha = runif(n = 10,min = 0.05,max = .125)
  pertb_direction = clo(c(100,rep(1,ncol(comp)-1)))
  c = simplexDataAugmentation::linearShift(data.frame(madison[,-2:-1]),
                                           a = alpha,alpha_n = alpha,pertubationDirection = pertb_direction,
  ...
  c = simplexDataAugmentation::linearShift(data.frame(madison[,-2:-1]),
                                           ...
                                           directionType = "centroid",evComp = 1)
  c = c$linAdjustedData
  s3 = data.frame(Dataset = "d3",Status = "C1",sample_n(c,size = 200,replace = F))
  ...
  ## Data Set 1
  x1 = (comp[1,])
  a = round(seq(-100,0,by = .1),1)  #<===== Input =======
  madison = foreach(A = a,.combine = rbind)%dopar%{
    ph = data.frame((compositions::clo(x1^A)))
    data.frame(Dataset = 1,Status = A,ph)
  }
...
p = performance %>%
  filter(Model == "LASSO" )
p = gather(p,key = "Partition","AUC",5:6)
ggplot(p,aes(Dims,AUC,col =Partition ))+
  geom_point()+
  geom_line()+
  facet_grid(.~Type)+
  theme_bw()
