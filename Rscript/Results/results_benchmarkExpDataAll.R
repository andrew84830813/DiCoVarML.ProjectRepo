...
fnames = dir("Results/Exp_Benchamark4/")
...
## all data
nm = (str_split(fnames,pattern = "_seed",simplify = T))
nm = unique(nm[,1])
...
for(f_name in nm){
  ...
  results = data.frame()
  for(i in f){
    ph = read_csv(file = paste0("Results/Exp_Benchamark4/",i))
...
## Compute DCV AUC i.e best of ensmeble vs ridge based on train auc
ph = results_all %>%
  filter(str_detect(Approach,"DCV-"))
modes = c("PLR_DCV-","ALR/PLR_DCV-","ALR_DCV-")

dcv.auc = data.frame()
seeds  =unique(ph$Seed)
for(s in seeds){
  for(a in modes){
    p = ph%>%
      filter(Seed==s & str_detect(Approach,pattern = a) )

    pp= p%>%
      group_by(Scenario,Dataset,Seed,Fold)%>%
      summarise(train_auc=max(train_auc))
    ppp =left_join(pp,p)
    ppp = dplyr::select(ppp,colnames(p))
    ppp$Approach = a
    dcv.auc = rbind(dcv.auc,ppp)
  }


}
dcv.auc$Approach = str_replace_all(string = dcv.auc$Approach,"-","")
modes = unique(dcv.auc$Approach)
...
for(s in seeds){
    ...
    ppp =left_join(pp,p)
    ppp = dplyr::select(ppp,colnames(p))
    ppp$Approach = "DiCoVarFS"
    dcv.ovr = rbind(dcv.ovr,ppp)
  }
...
results_all1 = results_all %>%
  filter(Approach %in% c("SELBAL","CLR-LASSO","Coda-LASSO"))


## add dcv results
results_all1 = rbind(results_all1,dcv.auc)
## add ovr results
#results_all1 = rbind(results_all1,dcv.ovr)




results_all1$Approach = factor(results_all1$Approach,levels = c("DiCoVarFS","PLR_DCV","ALR_DCV","ALR/PLR_DCV",
                                                              "CLR-LASSO","Coda-LASSO","SELBAL"))
results_all1 = results_all1 %>%
...
## Pairwsie comparsion
cx = list( c("PLR_DCV","CLR-LASSO"), c("PLR_DCV","Coda-LASSO"),c("PLR_DCV","SELBAL"),
           c("ALR_DCV","CLR-LASSO"), c("ALR_DCV","Coda-LASSO"),c("ALR_DCV","SELBAL"),
           c("ALR/PLR_DCV","CLR-LASSO"), c("ALR/PLR_DCV","Coda-LASSO"),c("ALR/PLR_DCV","SELBAL") )
...
for(d in ds){
  ...
  for(i in 1:length(cx)){
    ...
    ef = NA#rstatix::wilcox_effsize(data = dtt,formula = auc~label,alternative = "greater",ref.group = cx[[i]][1],paired = T)
...
ggplot(results_all1,aes(Approach,AUC,shape = Approach))+
  theme_bw()+
