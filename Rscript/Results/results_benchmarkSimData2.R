...
fnames = dir("Results/SimData3/")
fnames = dir("Results/SimData4/")
...
for(f_name in nm){
  ...
  results = data.frame()
  for(i in f){
    ph = read_csv(file = paste0("Results/SimData4/",i))
...
#   group_by(Scenario,Dataset,Seed,Approach,permuteLabel,shift_parm,sparsity) %>%
...
results_all1 = results_all %>%
  filter(Approach %in% c("SELBAL","CLR-LASSO","Coda-LASSO","PLR_PLR_DCV","ALR_ALR_DCV","ALR/PLR_ALR/PLR_DCV"))
...
unique(results_all1$Approach)
res = results_all1 %>%
  filter(Approach %in%  c("PLR_PLR_DCV","ALR_ALR_DCV","ALR/PLR_ALR/PLR_DCV","SELBAL","CLR-LASSO","Coda-LASSO")) %>%
  filter(shift_parm %in% c(4:6))

res$Approach = factor(res$Approach,levels = c("PLR_PLR_DCV","ALR_ALR_DCV","ALR/PLR_ALR/PLR_DCV","SELBAL","CLR-LASSO","Coda-LASSO"),
                      labels = c("PLR_DCV","ALR_DCV","ALR/PLR_DCV","SELBAL","CLR-LASSO","Coda-LASSO")
                      )
...
ggplot(results_all1,aes(Signal_Density,AUC,col = Approach))+
  ...
  stat_summary(fun.data = mean_se,geom = "pointrange",aes(group =Approach,col = Approach),width = .25,
               #position = position_dodge2(dw)
               )+
