...
selectionEnergy.scaled = function(inputData , optimizationMetric = NULL,
                                  ...
                                  nreps_energy = 1e4,impt_factor = 1e-7,
                                  eps = 1e-8,knn = 5,threshold = .25,
                                  targetFeats = NULL,scale_data = T,
                                  patience = 25,alpha_ = 0.05,useMST = T){
  ...
  lrs.scaled = lrs.train[,-1]
  ##scale data
  if(scale_data){
    pp = caret::preProcess(lrs.train[,-1],method = "scale")
    lrs.scaled <- predict(pp, lrs.train[,-1])  
  }
  ...
  if(useMST){
    ...
    ## Filter ratios
    allFeats = DiCoVarML::dcvRatioFilter(lrs_test = lrs.train,
                                         useKFN = T,k = knn,
                                         dcv_mat = dcv,
                                         lrs = lrs.train,
                                         th_percent = threshold,
                                         ytrain = lrs.train[, 1],
                                         impute_factor = impt_factor)
    keep = allFeats$dcvScore
    allFeats = allFeats$Dense$train
    dcv_=NULL
  }
...
# 
# 
# cospp  = function (inputData, optimizationMetric = NULL, dcv_useInfoGain = T, 
#                    dcv_nfold = 5, dcv_numRepeats = 1, nreps_energy = 10000, 
#                    eps = 1e-08, targetFeats = NULL, patience = 25, alpha_ = 0.05,useMST = T) {
#   Status = NULL
#   i = NULL
#   inputData$i = 1:nrow(inputData)
#   inputData = data.frame(Status = inputData[, 1], inputData[, 
#                                                             -1])
#   inputData = dplyr::arrange(.data = inputData, Status)
#   ids = inputData$i
#   inputData = dplyr::select(inputData, -i)
...
#   lrs.train = data.frame(Status = lrs.train[, 1], lrs.scaled)
#   dcv = diffCompVarRcpp::dcvScores(logRatioMatrix = lrs.train, 
#                                    includeInfoGain = dcv_useInfoGain, nfolds = dcv_nfold, 
#                                    numRepeats = dcv_numRepeats, rankOrder = T)
#   )
# 
# 
# 
...
# bd1 = vegan::permutest(mod, permutations = nreps_energy)
# if (is.null(optimizationMetric)) {
#   optimizationMetric = dplyr::if_else(bd1$tab$`Pr(>F)`[1] < 
#                                         alpha_, "combinedF", "scale")
#   message("dispersion p = ", round(bd1$tab$`Pr(>F)`[1], 
#                                    3), " ;  optmization is:  ", optimizationMetric)
# }
# baseSet = 1:3
# cn = colnames(allFeats)
# ph = subset(allFeats, select = cn[baseSet])
...
# if (optimizationMetric == "scale") {
#   d = parallelDist::parallelDist(as.matrix(ph))
#   enf = energy::disco(x = d, factors = lbs, distance = T, 
#                       R = 2, method = "discoF")
#   estat = enf$statistic
#   e_stat = estat
#   newF = estat
...
#   a.df = data.frame(Type = as.character(lbs))
#   d.compdat = parallelDist::parDist(as.matrix(ph), method = "euclidean")
#   pmv1 = vegan::adonis2(d.compdat ~ Type, data = a.df, 
#                         permutations = 2)
#   f1 = pmv1$F[1]
#   mod = vegan::betadisper(d.compdat, group = lbs)
#   bd1 = vegan::permutest(mod, permutations = 1)
...
# astat = data.frame()
# baseSet.list = list()
# astat = rbind(astat, data.frame(Value = maxF, optF = maxF, 
#                                 numFeats = length(baseSet)))
# baseSet.list[[1]] = baseSet
# bs = 2
# ss = 1
# end = ncol(allFeats)
...
#     else if (optimizationMetric == "combinedF") {
#       a.df = data.frame(Type = as.character(lbs))
#       d.compdat = parallelDist::parDist(as.matrix(ph), 
#                                         method = "euclidean")
#       pmv1 = vegan::adonis2(d.compdat ~ Type, data = a.df, 
#                             permutations = 2)
#       f1 = pmv1$F[1]
...
#     baseSet.list[[bs]] = baseSet
#     bs = bs + 1
#     astat = rbind(astat, data.frame(Value = newF, optF = maxF, 
#                                     numFeats = length(baseSet)))
#     if (!is.null(targetFeats)) {
#       if (targetFeats <= length(baseSet)) {
#         break
#       }
#     }
#     message("#Features = ", length(baseSet), ", maxF = ", 
#             round(maxF, 4), " , impTime = ", impTime)
#     if (diff_ > eps) {
#       impTime = 0
#     }
#     else {
...
# feats = allFeats[, pp]
# return(list(finalSubset = data.frame(Status = labels, feats), 
#             optimization_Metric = optimizationMetric, optimResult = astat, 
#             rawDCV = dcv_, retainedDCV = keep, mstSubset = allFeats))
# }
